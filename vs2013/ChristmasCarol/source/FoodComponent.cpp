...
void FoodComponent::Init()
{
	...
	switch (m_type)
	{
		...
		for (int i = 0; i < m_state + 1; i++)
		{
			...
			{
			case 0:
				batch->AddSprite(&m_objects.back()->transform, "plate1.png");
				m_objects.back()->transform.SetScale(Randomizer::GetFloat(0.4, 0.5)*1.5);
				break;
			default:
				m_objects.back()->transform.SetPosition(Randomizer::InsideCircle(50));
	...
	case 2:
		m_state = 3;
		{
			...
			for (int i = 0; i < m_state + 1; i++)
			{
				...
				{
				case 0:
					if (r < 0.5)batch->AddSprite(&m_objects.back()->transform, "plate1.png");
					else batch->AddSprite(&m_objects.back()->transform, "plate2.png");
		...
		m_state = 3;
		{
			...
			for (int i = 0; i < m_state + 1; i++)
			{
				...
				case 0:
					...
					break;
				case 1:
					if (r < 0.5)batch->AddSprite(&m_objects.back()->transform, "turkey3.png");
					else batch->AddSprite(&m_objects.back()->transform, "ham3.png");
					m_objects.back()->transform.SetScale(r2);
					break;
				case 2:
					if (r < 0.5)batch->AddSprite(&m_objects.back()->transform, "turkey2.png");
					else batch->AddSprite(&m_objects.back()->transform, "ham2.png");
					m_objects.back()->transform.SetScale(r2);
					break;
				case 3:
					if (r < 0.5)batch->AddSprite(&m_objects.back()->transform, "turkey1.png");
					else batch->AddSprite(&m_objects.back()->transform, "ham1.png");
					m_objects.back()->transform.SetScale(r2);
					break;
				}
			}
